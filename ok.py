# Edge Impulse - OpenMV FOMO Object Detection Example (Fixed for firmware 4.8.1)
#
...
def fomo_post_process(model, inputs, outputs):
    ...
    for i in range(oc):
        ...
        for y in range(oh):
            row_offset = y * ow
            for x in range(ow):
                val = int(channel[y, x] * 255 + 0.5)  # 四舍五入
                if val < 0:
                    val = 0
                elif val > 255:
                    val = 255
        ...
        # 创建灰度图像
        img = image.Image(ow, oh, image.GRAYSCALE, buffer=buf, copy_to_fb=False)
...
while True:
    ...
    for i, detection_list in enumerate(results):
        if i == 0:            # 背景类跳过
            continue
