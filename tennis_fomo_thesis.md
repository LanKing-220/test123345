...
```mermaid
flowchart TD
	A[摄像头采集图像] --> B[图像预处理（缩放、色彩转换）]
	B --> C[FOMO模型推理（TFLite）]
	C --> D1[连通域分析（Blob）]
	D1 --> E1[初步目标中心点提取]
	E1 --> F1[多线索融合处理]
	F1 --> G1[颜色Blob检测（自适应LAB）]
	F1 --> G2[边缘检测+Canny]
	F1 --> G3[Hough圆检测]
	G1 --> H[目标半径估计]
	G2 --> H
	...
	J --> K[可视化与结果输出]
	K --> L[下游分析/控制]
	style F1 fill:#e0f7fa,stroke:#00796b,stroke-width:2px
...
图4-1展示了本系统在OpenMV端的完整识别与后处理流程。该流程自摄像头采集图像开始，经过FOMO模型推理、多线索融合、目标半径估计、轨迹管理等环节，最终实现了高鲁棒性和高精度的网球目标检测。
